...
def create_visualizations():
    ...
    tpa = db.read_csv(data_dir / "TrafficPerAirport.csv")
    tpt = db.read_csv(data_dir / "TrafficPerTerritory.csv")
    airports = db.read_csv(data_dir / "Airport.csv")
    airservice = db.read_csv(data_dir / "AirService.csv")
    aircraftmov = db.read_csv(data_dir / "AircraftMovement.csv")
    calendarmonth = db.read_csv(data_dir / "CalendarMonth.csv")

    tpa = db.sql("""
        SELECT tpa.*, cm.MonthStartDate
        FROM tpa
        INNER JOIN calendarmonth cm USING (MonthId)
    """).df()

    tpt = db.sql("""
        SELECT tpt.*, cm.MonthStartDate
        FROM tpt
        INNER JOIN calendarmonth cm USING (MonthId)
    """).df()

    tpa = tpa.rename(columns={"MonthStartDate": "Month"})
    tpt = tpt.rename(columns={"MonthStartDate": "Month"})
    ...
    create_air_traffic_html(db.sql(f"SELECT AirService, AircraftMovement, Month, a.AirportName AS BaseAirport, a.Latitude AS BaseLatitude, a.Longitude AS BaseLongitude, \
            ...
            output_file=docs_dir / "air_traffic.html",
    ...
    create_air_traffic_line_graph(db.sql(f"SELECT Month, a.AirportName AS BaseAirport, \
            ...
            output_file=docs_dir / "line_graph.html")

    predictions = db.read_csv(data_dir / "predictions" / "Predictions.csv")

    predictions = db.sql(f"SELECT * FROM predictions WHERE Month >= '2023-01-01'")

    create_prediction_line_graph(
        predictions.df(),
        output_file=docs_dir / "prediction_graph.html",
    )
    ...
    ").df(), output_file=docs_dir / "airport_bar_chart.html")
