...
from docs.pipeline_update_plots import pipeline_update_plots
...
with DAG(
    ...
    task_plots = PythonOperator(
        task_id="update_plots",
        python_callable=pipeline_update_plots,
    )
    ...
    run_commit_script = BashOperator(
        ...
        bash_command="""
            ...
            git add . && git commit -m "Automatic commit with airflow"
            git push
        """,
    )

    # Set execution order
    task_airport >> task_territory >> task_models >> task_plots >> run_commit_script